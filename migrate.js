...
async function migrate() {
  ...
  // Estendi tabella products esistente (aggiunge campi solo se non esistono)
  await pool.query(`
  ...
  // Tabella documenti/foto (riferimenti a Cloudinary)
  await pool.query(`
  ...
  // Tabella impostazioni budget (un record per ambiente o totale)
  await pool.query(`
    CREATE TABLE IF NOT EXISTS budget_settings (
      id            SERIAL PRIMARY KEY,
      ambito        TEXT UNIQUE NOT NULL,
      tipo          TEXT NOT NULL,
  ...
  await pool.query(`
    ...
    CREATE TRIGGER budget_updated_at BEFORE UPDATE ON budget_settings
      FOR EACH ROW EXECUTE FUNCTION update_updated_at();
  `);
